...
function sendMail($email, $reset_token)
{
    ...
    try {
        ...
        $mail->Subject = 'Password Reset link from Farming Website';
        $mail->Body    = "We got a request from you to reset your password! <b> 
        Click the link below : <br>
        <a href='http://localhost/pradumnagothi/updatepassword.php?email=$email&reset_token=$reset_token'>
        Reset Password
        </a>";
...
if (isset($_POST['send-reset-link'])) 
{
    ...
    if ($result) {
        if (mysqli_num_rows($result) == 1) {
            ...
            $date = date("y-m-d");
            $query = "UPDATE `registered_user` SET `resettoken`='$reset_token',`resettokenexpire`='$date' WHERE `email` = '$_POST[email]'";
